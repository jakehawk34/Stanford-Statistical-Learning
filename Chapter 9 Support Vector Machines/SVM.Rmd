...
As mentioned in the chapter, the plot function is somewhat crude, and plots X2 on the horizontal axis (unlike what it would automatically do for a matrix). Let's see how we might make our own plot.

The first thing we will do is make a grid of values for X1 and X2. We will write a function to do that, in case we want to reuse it. It uses the handy function `expand.grid`, and produces the coordinates of `n*n` points on a lattice covering the domain of `x`. Having made the latice, we make a prediction at each point on the lattice. We then plot the lattice, color-coded according to the classification. Now we can see the decision boundary.
...
These data are also two dimensional. Let's plot them and fit a nonlinear SVM, using a radial kernel.
...
contour(px1, px2, matrix(func, 69, 99), level=0, add=TRUE, col="green")
