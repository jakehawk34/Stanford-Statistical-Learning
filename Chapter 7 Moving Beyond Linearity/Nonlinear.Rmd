...
Here we explore the use of nonlinear models using tools in R
...
There are other more direct ways of doing this R. For example
...
Here `I()` is a "wrapper" function; we need it because `age^2` means something to the formal language, while `I(age^2)` is protected.
The coefficients are different to those we got before!
However, the fits are the same:
...
Now we git a logistic regression model to a binary response variable, constructed from `wage`. We code the big earners (`<250k`) as 1, else 0.
...
(Here we have used the abilty of MarkDown to interpret TeX expressions.)
...
The smoothing splines do not require knot selection, but it does have a smoothing parameter, which can conveniently be specified via the effective degrees of freedom or `df`.
